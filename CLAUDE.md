...
1. **Swatch sets and label presets** — Polish terrain (`TERENY`, 19 types) and POI (`POI`, 23 types) palettes, each entry mapping a display label/symbol to a Mudlet environment ID, plus the label presets `MIASTA` / `WIOSKA` (gold Segoe UI caps, translucent frame, transparent background, cities a size up) and `STATKI` (yellow, left-aligned, no frame) — all three size their box to their own text, so multi-line names fit.
