 ...
 * node-mudlet-map-binary-reader. Deviations from the reference:
 *   - reads the extra `ObjectTbl` columns we round-trip (Cost, Enabled,
 *     UserStr, UserInt, Content) and `ExitTbl.Distance`;
 *   - skips rows flagged `Deleted`;
 *   - reads the `ArkadiaMetaTbl` sidecar into `CmudMap.meta`.
 */
import type { QueryFn } from './sqlite';
import type {
  CmudDirection,
  CmudExit,
  CmudLabel,
...
export function parseCmudMap(query: QueryFn): CmudMap {
  ...
  const meta = parseMeta(query, tables);

  return { zones, rooms, exits, directions, notes, styles, labels, meta, unknownColumns };
}
...
/** cMUD stores booleans as `'Y'`/`'N'` text (Access heritage); tolerate numbers too. */
...
function parseRooms(
  ...
  for (const row of rows) {
    ...
    const costCol = pick(row, 'Cost');
    const enabledCol = pick(row, 'Enabled');
    const userStrCol = pick(row, 'UserStr');
    const userIntCol = pick(row, 'UserInt');
    const contentCol = pick(row, 'Content');
    const deletedCol = pick(row, 'Deleted');
    if (!idCol) continue;
    if (deletedCol && toBool(row[deletedCol], false)) continue;
    const id = toNum(row[idCol]);
    out[id] = {
      id,
      ...
      cost: costCol ? toNum(row[costCol]) : undefined,
      enabled: enabledCol ? toBool(row[enabledCol], true) : undefined,
      userStr: userStrCol ? toStr(row[userStrCol]) : undefined,
      userInt: userIntCol ? toNum(row[userIntCol]) : undefined,
      content: contentCol ? toStr(row[contentCol]) : undefined,
    };
    recordUnused(
      unknown,
      'ObjectTbl',
      ...
      new Set(
        [
          idCol, zoneCol, nameCol, descCol, xCol, yCol, zCol, styleCol, kindCol,
          flagsCol, colorCol, costCol, enabledCol, userStrCol, userIntCol,
          contentCol, deletedCol,
...
function parseExits(
  ...
  for (const row of rows) {
    ...
    const flagsCol = pick(row, 'Flags');
    const distanceCol = pick(row, 'Distance');
    if (!exitIdCol || !fromCol) continue;
    const nameVal = nameCol ? toStr(row[nameCol]) : '';
    const lineCols = ['X0', 'Y0', 'Z0', 'X1', 'Y1', 'Z1'].map((c) => pick(row, c));
    const [x0, y0, z0, x1, y1, z1] = lineCols.map((c) => (c ? toNum(row[c]) : 0));
    const line = x0 || y0 || z0 || x1 || y1 || z1 ? { x0, y0, z0, x1, y1, z1 } : undefined;
    out.push({
      ...
      distance: distanceCol ? toNum(row[distanceCol]) : undefined,
      line,
    });
    recordUnused(
      unknown,
      ...
      new Set(
        [
          exitIdCol, exitIdToCol, fromCol, toCol, dirCol, dirToCol, kindCol,
          nameCol, flagsCol, distanceCol, ...lineCols,
        ].filter((c): c is string => !!c),
      ),
    );
  }
...
function parseNotes(
  ...
  for (const row of rows) {
    ...
    recordUnused(
      ...
      'NoteTbl',
      row,
      new Set([objCol, catCol, textCol, deletedCol].filter((c): c is string => !!c)),
    );
  }
  return out;
}
...
/** Key/value sidecar written by this editor's serializer; absent in real cMUD files. */
function parseMeta(query: QueryFn, tables: Set<string>): Record<string, string> {
  const out: Record<string, string> = {};
  if (!tables.has('ArkadiaMetaTbl')) return out;
  for (const row of query('SELECT * FROM ArkadiaMetaTbl')) {
    const keyCol = pick(row, 'Key');
    const valCol = pick(row, 'Value');
    if (!keyCol || !valCol) continue;
    out[toStr(row[keyCol])] = toStr(row[valCol]);
  }
  return out;
}
